...
class PersonAPI(ApiRequests):
    DEV_DOMAIN = Urls.DEV_DOMAIN.value
    PERSON_ENDPOINT = EndPoints.PERSON.value

    def get_all_persons_dict_length(self):
        response = self.get(f'{self.DEV_DOMAIN}{self.PERSON_ENDPOINT}')
        response_object = response.json()
        return len(response_object)

    def get_person(self, person_id):
        response = self.get(f'{self.DEV_DOMAIN}{self.PERSON_ENDPOINT}/{person_id}')
    ...
    def create_new_person(self):
        ...
        response = self.post(f'{self.DEV_DOMAIN}{self.PERSON_ENDPOINT}', data=json_data)
        return response
    ...
    def modify_person(self, person_id: int, modified_data_dict: Union[Dict[str, T], str]):
        response = self.put(f'{self.DEV_DOMAIN}{self.PERSON_ENDPOINT}/{person_id}', data=modified_data_dict)
        return response

    def delete_person(self, person_id: int):
        response = self.delete(f'{self.DEV_DOMAIN}{self.PERSON_ENDPOINT}/{person_id}')
        return response
